...
import { mapPostalToInsertEmail } from "./app/ingestion.ts";
...
test("Ingestion Module — 字段映射不丢属性且通过 schema 校验", () => {
  ...
  const inserted = mapPostalToInsertEmail(mail, { from: "sender@example.com", to: "alice@wenxi.dev" }, now, "test-id-1");
...
test("Ingestion Module — 缺少 messageId 时 schema 校验失败", () => {
  const now = new Date();
  const mail: any = { headers: [], from: { address: "a@b.com", name: "" }, subject: "s", messageId: undefined };
  assert.throws(() => mapPostalToInsertEmail(mail, { from: "a@b.com", to: "b@wenxi.dev" }, now, "id2"));
});
